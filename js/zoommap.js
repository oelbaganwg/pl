...
(function($) {
	$.fn.zoommap = function(settings) {
		settings = $.extend({
			...
			showReturnLink: true,
			returnId: 'returnlink',
			returnText: 'Click Anywhere To Zoom Out '
		}, settings);
		...
		$(this).each(function(){
			...
			function loadRegionData(region){
				...
				map.load(url, {}, function(){
					...
					$(this).children('a.bullet').each(function(){
						...
						$(this).css({display:'none', left: addpx(Number(coords[0]) - rempx(settings.bulletWidthOffset)), top: addpx(Number(coords[1]) - rempx(settings.bulletHeightOffset))})
							   .hide()
			...
			function showPopup(id, leftbul, topbul){
				...
				$(settings.popupCloseSelector).mouseleave(function(){
					$(this).parent().fadeOut();
				});
			}
			...
			if(hash.length > 0)
				...
				//$('.officeBullet').css({display:'block');
				//$('.warehouseBullet').css({display:'none');
				//$('.retailBullet').css({display:'none');
				//$('.aviationBullet').css({display:'none');
				//$('.medicalBullet').css({display:'none');
			}
